...
class App extends Component {
  ...
  onUpdateActiveTabId = newTabId => {
    this.setState({activeTabId: newTabId})
  }

  onClickListImage = activeImageId => {
    ...
    if (activeImageId === imagesList[index].id) {
      ...
      this.setState({thumbnailAndImageAreSame: true})
    }
  }
  ...
  render() {
    ...
    const matchImg = imagesList[index].imageUrl
    console.log(thumbnailAndImageAreSame)
        ...
        <div className="bg-container">
          {!thumbnailAndImageAreSame ? (
            <div className="background-container">
              <img src={matchImg} alt="match" className="image-to-be-matched" />
              <ul className="ul-tab-container">
                {tabsList.map(eachTab => {
                  const btnClassName =
                    activeTabId === eachTab.tabId ? 'active-tab-btn' : ''
                  return (
                    <li key={eachTab.tabId} className="tab-list-item">
                      <button
                        type="button"
                        className={`tab-btn ${btnClassName}`}
                        onClick={() => this.onUpdateActiveTabId(eachTab.tabId)}
                      >
                        {eachTab.displayText}
                      </button>
                    </li>
              ...
              <ul className="ul-container">
                {filteredImages.map(eachImage => (
                  <li className="list-item" key={eachImage.id}>
                    <button
                      type="button"
                      className="list-btn"
                      onClick={() => this.onClickListImage(eachImage.id)}
                    >
                      <img
